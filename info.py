...
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '6258598643').split()]
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1001995317850').split()]
...
LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1002091770837'))
...
URL = "https://auto-filter-vm-bot.onrender.com".format(FQDN) if ON_HEROKU or NO_PORT else \
    "https://auto-filter-vm-bot.onrender.com/".format(FQDN, PORT)
...
if HAS_SSL:
    URL = "https://auto-filter-vm-bot.onrender.com/".format(FQDN)
else:
    URL = "https://auto-filter-vm-bot.onrender.com/".format(FQDN)
